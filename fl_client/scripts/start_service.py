...
def submit_gradient():
    ...
    try:
        ...
        request_size_error = None
        try:
            request_size_bytes = len(json.dumps(submission_data, default=str).encode("utf-8"))
        except Exception as size_error:
            request_size_error = str(size_error) or type(size_error).__name__
            print(
                "[M3] Warning: could not calculate exact submission request size; "
                f"continuing with submit. Reason: {request_size_error}"
            )
        ...
        try:
            ...
            record_metric_event(
                ...
                payload={
                    ...
                    "request_size_error": request_size_error,
