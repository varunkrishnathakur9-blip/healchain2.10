"""
Create label-skew non-IID client splits for a Chest X-ray training dataset.
...
Output is written beside the original dataset as:
    <original_name>_non_iid_<n>_clients/client_01
    <original_name>_non_iid_<n>_clients/client_02
    ...
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Split a Chest X-ray dataset into label-skew non-IID client datasets."
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.8,
        help="Fraction of each class assigned to its primary clients",
    )
...
def main() -> None:
    ...
    splits = label_skew_non_iid_split(
        dataset_info.samples,
        num_clients,
        args.seed,
        dominant_fraction=args.dominant_fraction,
    )
    output_root = make_output_root(source_path, "non_iid", num_clients, args.output_dir)
    summary_path = write_client_datasets(
        dataset_info=dataset_info,
        splits=splits,
        output_root=output_root,
        split_type="label_skew_non_iid",
        strategy=(
            f"Label-skew split with dominant_fraction={args.dominant_fraction}. "
            "Most samples of each class are assigned to class-primary clients; "
            "the remainder is spread to other clients."
        ),
        seed=args.seed,
    )
